...
describe("loan calculation service", () => {
  ...
  it("returns total interest and remaining income from real supplied inputs", () => {
    ...
    expect(result.totalRepayment).toBeCloseTo(106618.56, 2);
    expect(result.totalInterest).toBeCloseTo(6618.56, 2);
    expect(result.remainingIncome).toBeCloseTo(16115.12, 2);
    expect(result.repaymentToIncomePercent).toBeCloseTo(35.5396, 3);
  });
