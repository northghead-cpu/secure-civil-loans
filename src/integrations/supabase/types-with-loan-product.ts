...
type AutomationRules = {
  Row: { action: string; config: BaseDatabase['public']['Tables']['profiles']['Row']['user_id'] extends string ? import('./types').Json : never; created_at: string; description: string; enabled: boolean; id: string; name: string; run_count: number; trigger: string; updated_at: string };
  Insert: { action: string; config?: import('./types').Json; created_at?: string; description?: string; enabled?: boolean; id?: string; name: string; run_count?: number; trigger: string; updated_at?: string };
  Update: { action?: string; config?: import('./types').Json; created_at?: string; description?: string; enabled?: boolean; id?: string; name?: string; run_count?: number; trigger?: string; updated_at?: string };
  Relationships: [];
};
...
type SystemSettings = {
  Row: { key: string; updated_at: string; updated_by: string | null; value: import('./types').Json };
  Insert: { key: string; updated_at?: string; updated_by?: string | null; value?: import('./types').Json };
  Update: { key?: string; updated_at?: string; updated_by?: string | null; value?: import('./types').Json };
  Relationships: [];
};

type Incidents = {
  Row: {
    acknowledged_at: string | null; acknowledged_by: string | null; audit_log_id: string | null; correlation_id: string | null;
    created_at: string; id: string; investigating_at: string | null; investigating_by: string | null; occurred_at: string;
    operation: string; resolution_code: string | null; resolved_at: string | null; resolved_by: string | null; severity: string;
    source: string; status: string; summary: string; updated_at: string;
  };
  Insert: {
    acknowledged_at?: string | null; acknowledged_by?: string | null; audit_log_id?: string | null; correlation_id?: string | null;
    created_at?: string; id?: string; investigating_at?: string | null; investigating_by?: string | null; occurred_at?: string;
...
type ExtendedTables = Omit<
  ...
  'audit_logs' | 'automation_rules' | 'bank_products' | 'incidents' | 'lender_commission_settings' | 'system_settings'
> & {
...
type ExtendedPublic = Omit<BaseDatabase['public'], 'Tables' | 'Functions'> & {
