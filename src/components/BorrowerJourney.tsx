...
const BorrowerJourney = ({
  ...
  const steps: { label: string; description: string; state: StepState }[] = [
    ...
    },
    {
      ...
      description: kycRejected
        ? "Action required: resubmit your verification information."
        : kycInReview
          ? "Your verification information is being reviewed."
          : kycComplete
            ? "Your identity and employment information is verified."
            : "Verify your identity and employment to unlock eligible offers.",
    ...
    },
    {
      ...
      description: kycComplete
        ? applicationCount > 0
          ? "Review the offers available to you and your applications."
          : "Verified lender offers will appear here when available."
        : "Available after verification is complete.",
      state: kycComplete ? "current" : "upcoming",
    },
    {
      label: "Choose a financial institution",
      description: "Select the lender and product you want to pursue.",
      state: kycComplete && applicationCount > 0 ? "current" : "upcoming",
    },
    {
  ...
  const getIcon = (state: StepState) => {
    if (state === "complete") return <CheckCircle2 className="h-5 w-5 text-success" />;
    if (state === "attention") return <AlertCircle className="h-5 w-5 text-destructive" />;
    if (state === "current") return <Clock3 className="h-5 w-5 text-primary" />;
