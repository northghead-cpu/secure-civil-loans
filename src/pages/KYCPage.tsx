...
import { Upload, CreditCard, FileText, FileSignature, CheckCircle2, ArrowRight, ArrowLeft, ShieldCheck, Info, Loader2 } from "lucide-react";
...
const KYCPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    const filesToCheck = [{ field: "NRC", file: formData.nrcFile }, { field: "Government ID", file: formData.govIdFile }, { field: "Payslip", file: formData.payslipFile }];
    for (const { field, file } of filesToCheck) { if (file && file.size > MAX_FILE_BYTES) { toast.error(`${field} file exceeds the 10MB limit.`); return; } }
    setSubmitting(true);
    try {
