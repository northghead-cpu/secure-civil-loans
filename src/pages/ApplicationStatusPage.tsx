...
interface ApplicationHandoff { id: string; lender_name: string; product_name: string | null; requested_amount: number | null; term_months: number | null; interest_rate: number | null; estimated_monthly_repayment: number | null; total_repayment: number | null; status: string; created_at: string; updated_at: string; }
const statusOrder = ["authorized", "preparing", "sent_to_lender", "lender_review", "approved"];
...
const ApplicationStatusPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchHandoff = async () => {
      const { data } = await supabase.from("application_handoffs").select("id, lender_name, product_name, requested_amount, term_months, interest_rate, estimated_monthly_repayment, total_repayment, status, created_at, updated_at").eq("user_id", user.id).neq("status", "pending_authorization").order("updated_at", { ascending: false }).limit(1).maybeSingle();
      setHandoff(data as ApplicationHandoff | null);
      setLoading(false);
    };
    void fetchHandoff();
    const channel = supabase.channel(`application-handoff-${user.id}`).on("postgres_changes", { event: "*", schema: "public", table: "application_handoffs", filter: `user_id=eq.${user.id}` }, fetchHandoff).subscribe();
  ...
  const currentIndex = useMemo(() => {
    ...
    if (["approved", "declined", "disbursed"].includes(handoff.status)) return 4;
  ...
  return (
    <div className="min-h-screen bg-background"><Navbar /><main className="pt-24 pb-16"><div className="container mx-auto px-4 lg:px-8 max-w-4xl space-y-6">
      ...
      {!handoff ? <Card><CardContent className="p-8 text-center"><FileCheck2 className="w-10 h-10 mx-auto mb-4 text-muted-foreground"/><h2 className="text-xl font-display font-semibold">No active application</h2><p className="text-muted-foreground mt-2">Choose a verified loan offer to begin an application with a participating financial institution.</p><Button className="mt-6" onClick={() => navigate("/compare")}>Compare loan options</Button></CardContent></Card> : <>
        ...
        <Card><CardHeader><CardTitle className="text-lg font-display">Application journey</CardTitle></CardHeader><CardContent><div className="space-y-5">{[["Offer selected", "You've selected this financial institution.", CheckCircle2], ["Information sharing authorized", "You authorized Riverbanc to share the agreed information.", Send], ["Application prepared and sent", "Riverbanc prepares and sends your authorized information.", FileCheck2], ["Financial institution review", "The financial institution reviews your application.", Clock3], ["Decision", "The financial institution makes the lending decision.", isDecision && handoff.status === "declined" ? XCircle : CheckCircle2]].map(([title, description, Icon], index) => { const complete = index < currentIndex || (index === 4 && isDecision); const active = index === currentIndex && !isDecision; const StepIcon = Icon as typeof CheckCircle2; return <div key={String(title)} className="flex items-start gap-3"><div className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border ${complete ? "border-primary bg-primary/10 text-primary" : active ? "border-primary text-primary" : "border-border text-muted-foreground"}`}><StepIcon className="h-4 w-4"/></div><div><p className={`font-medium ${active ? "text-primary" : "text-foreground"}`}>{String(title)}</p><p className="text-sm text-muted-foreground">{String(description)}</p></div></div>; })}</div></CardContent></Card>
        {handoff.status === "additional_information_requested" && <Card className="border-warning/40"><CardContent className="p-5 flex items-start gap-3"><MessageSquareText className="w-5 h-5 text-warning mt-0.5 shrink-0"/><div><p className="font-medium">Action required</p><p className="text-sm text-muted-foreground mt-1">The financial institution has requested additional information. Review the request before continuing.</p></div></CardContent></Card>}
