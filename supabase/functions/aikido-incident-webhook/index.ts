...
function severity(value: unknown): "critical" | "high" | "medium" | "low" {
...
Deno.serve(async (req) => {
  ...
  // Aikido signs the JSON-stringified payload and includes dispatched_at for
  // replay protection. This intentionally follows their documented order.
  const canonicalPayload = JSON.stringify(payload);
  const expected = await hmacSha256(secret, canonicalPayload);
  if (!secureEqual(signature.trim().toLowerCase(), expected)) {
    return new Response(JSON.stringify({ error: "Invalid signature" }), { status: 401, headers: corsHeaders });
  }

  const dispatchedAt = Number(payload.dispatched_at);
  const now = Math.floor(Date.now() / 1000);
  if (!Number.isFinite(dispatchedAt) || Math.abs(now - dispatchedAt) > MAX_DISPATCH_AGE_SECONDS) {
    return new Response(JSON.stringify({ error: "Stale webhook" }), { status: 401, headers: corsHeaders });
  }
  ...
  const summary = clamp(
    `${eventType}${issueId !== undefined ? ` — issue ${String(issueId)}` : ""}${data.type ? ` (${String(data.type)})` : ""}`,
  );
  ...
  const { data: incidentId, error } = await supabase.rpc("record_external_incident", {
    ...
    p_severity: severity(data.severity),
    p_summary: summary,
    p_correlation_id: issueId === undefined ? null : `aikido:${String(issueId)}`,
  });

  if (error) {
    console.error("[aikido-incident-webhook] ingestion failed", error.message);
